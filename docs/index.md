...
`pak``::`[`pak`](https://pak.r-lib.org/reference/pak.html)`(``"kvasilopoulos/econdata"``)`
...
\
`catalog``[``, `[`c`](https://rdrr.io/r/base/c.html)`(``"key"``, ``"category"``, ``"frequency"``, ``"start"``, ``"end"``, ``"nrow"``, ``"ncol"``)``]`\
`#> # A tibble: 9 × 7`\
...
\
`# Datasets tagged "var"`\
`catalog``$``key``[`[`sapply`](https://rdrr.io/r/base/lapply.html)`(`[`strsplit`](https://rdrr.io/r/base/strsplit.html)`(``catalog``$``tags``, ``"|"``, fixed ``=`` ``TRUE``)``, ``function``(``t``)`` ``"var"`` `[`%in%`](https://rdrr.io/r/base/match.html)` ``t``)``]`\
...
\
[`read.csv`](https://rdrr.io/r/utils/read.table.html)`(``"https://kvasilopoulos.github.io/econdata/data/sw2001.csv"``)`
...
\
`sources``[`[`grepl`](https://rdrr.io/r/base/grep.html)`(``"monetary-policy"``, ``sources``$``tags``)``, `[`c`](https://rdrr.io/r/base/c.html)`(``"name"``, ``"coverage"``)``]`\
`#> # A tibble: 8 × 2`\
`#>   name                                                             coverage    `\
`#>   <chr>                                                            <chr>       `\
...
- We follow the [tidyverse
  principles](https://kvasilopoulos.github.io/econdata/CONTRIBUTING.md);
  see also
  [PKG_CONTRIBUTING.md](https://kvasilopoulos.github.io/econdata/PKG_CONTRIBUTING.md).

Please note that the ‘econdata’ project is released with a [Contributor
Code of
Conduct](https://kvasilopoulos.github.io/econdata/CODE_OF_CONDUCT.md).
